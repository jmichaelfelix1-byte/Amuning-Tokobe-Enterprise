...
try {
    ...
    $stmt = $conn->prepare("SELECT po.id, po.status as order_status, COALESCE(p.status, 'unpaid') as payment_status
    ...
    $delete_payment_stmt = $conn->prepare("DELETE FROM payments WHERE reference_id = ? AND payment_type = 'printing_order'");
    $delete_payment_stmt->bind_param("i", $order_id);
    $delete_payment_stmt->execute();
    $delete_payment_stmt->close();

    // Delete the printing order
    $delete_order_stmt = $conn->prepare("DELETE FROM printing_orders WHERE id = ?");
